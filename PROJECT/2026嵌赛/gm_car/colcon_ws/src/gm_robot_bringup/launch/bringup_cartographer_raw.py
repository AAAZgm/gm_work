...
def generate_launch_description():
    ...
    cartographer_config = os.path.join(bringup_dir, 'config', 'cartographer_raw.lua')
    ...
    cartographer_node = launch_ros.actions.Node(
        ...
        arguments=['-configuration_directory',os.path.dirname(cartographer_config),#取目录
                   '-configuration_basename', 'cartographer_raw.lua'],
        remappings=[
        ('scan', '/scan'),      # 激光话题
        ('odom', '/odom'),      # 里程计话题
    ]
    ...
    cartographer_occupancy_grid_node = launch_ros.actions.Node(
        ...
        parameters=[{'use_sim_time': launch.substitutions.LaunchConfiguration('use_sim_time')},
                    {'resolution': 0.05},
                    {'publish_period_sec': 0.25}],  # 添加：降低发布频率
    )
   ...
   # odom2tf_delay = launch.actions.TimerAction(period=1.0, actions=[odom2tf_node])
    cartographer_delay = launch.actions.TimerAction(period=1.0, actions=[cartographer_node])
