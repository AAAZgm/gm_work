 ...
 *    3. 网页上有 4 个滑块, 分别控制机械臂的 4 个关节舵机
 *    4. 上电时所有舵机自动回到 90° (机械臂竖直中立位置)
 *    5. 支持保存/加载预设姿态, 方便重复执行动作
 *    6. 读取 AHT30 温湿度传感器, 网页和 ROS 话题实时显示
 *  接线说明:
 *    - 4 个舵机的信号线分别接到 GPIO 6~9 (需根据实际修改)
 *    - AHT30 传感器: SDA→GPIO4, SCL→GPIO5 (I2C)
...
#include <Adafruit_AHTX0.h> // AHT30 温湿度传感器库
// ========== micro-ROS 头文件 ==========
...
std_msgs__msg__Float32MultiArray angle_pub_msg;           // 发布消息: 当前 4 个关节角度
...
rcl_publisher_t temp_humidity_publisher;                   // 温湿度话题发布者
std_msgs__msg__Float32MultiArray temp_humidity_pub_msg;    // 发布消息: [温度, 湿度]

rcl_timer_t timer1;
rcl_timer_t timer2;
// AHT30 温湿度传感器
Adafruit_AHTX0 aht;
float currentTemp = 0.0;    // 当前温度 (°C)
float currentHumidity = 0.0; // 当前湿度 (%RH)
bool ahtReady = false;       // AHT30 是否初始化成功
...
const char *WIFI_SSID     = "Gm_wifi";   // WiFi 名称, 需改成你手机热点的名称
const char *WIFI_PASSWORD = "123456789";   // WiFi 密码, 需改成你手机热点的密码

// =================================================================
//  二、舵机硬件配置
//  4DOF = 4 个自由度, 即 4 个舵机分别控制机械臂的不同关节
//  使用 ESP32-S3 的 LEDC 硬件 PWM 直接驱动, 每路完全独立
// =================================================================

#define NUM_SERVOS 4  // 宏定义: 舵机总数量, 4 个自由度 = 4 个舵机

// 舵机信号线连接的 GPIO 引脚号
// 格式: {底座, 肩部, 肘部, 夹爪}
// 注意: 不要用 GPIO 0~3, 这些是启动引脚/串口引脚
const int SERVO_PINS[NUM_SERVOS] = {6, 7, 15, 16};
// 第0个舵机(底座)接 GPIO6, 第1个(肩部)接 GPIO7, 依次类推

// 每个舵机对应一个独立的 LEDC 通道号 (0~3, 每个通道控制一个舵机)
const int SERVO_CHANNELS[NUM_SERVOS] = {0, 1, 2, 3};
// LEDC 通道 0~3
...
void temp_humid_callback(rcl_timer_t* timer, int64_t last_call_time);

void angle_pub_callback(rcl_timer_t* timer, int64_t last_call_time);

// micro-ROS 状态标志
volatile bool micro_ros_initialized = false;
volatile bool micro_ros_connected = false;

void micro_ros_init() {
    Serial.println("\n[micro-ROS] 开始初始化...");

    IPAddress agent_ip(10, 210, 62, 71);  // ★ 改成你上位机的 IP
    size_t agent_port = 8888;
    Serial.printf("[micro-ROS] Agent IP: %s:%d\n",
                  agent_ip.toString().c_str(), agent_port);

    // 设置传输层 (复用 setup() 中已建立的 WiFi 连接, 不调用 WiFi.begin)
    Serial.println("[micro-ROS] 设置传输层...");
    ...
    // 初始化 support
    Serial.println("[micro-ROS] 初始化 support...");
    rcl_ret_t ret = rclc_support_init(&support, 0, NULL, &allocator);
    if (ret != RCL_RET_OK) {
        Serial.printf("[micro-ROS] support_init 失败: %d\n", ret);
        return;
    }

    // 创建节点
    Serial.println("[micro-ROS] 创建节点...");
    ret = rclc_node_init_default(&node, "esp32_servo_controller", "", &support);
    if (ret != RCL_RET_OK) {
        Serial.printf("[micro-ROS] node_init 失败: %d\n", ret);
        return;
    }

    // --- 话题发布者: /arm_joint_angles ---
    Serial.println("[micro-ROS] 创建发布者 /arm_joint_angles ...");
    memset(&angle_pub_msg, 0, sizeof(angle_pub_msg));
    angle_pub_msg.data.size = NUM_SERVOS + 1;
    angle_pub_msg.data.capacity = NUM_SERVOS + 1;
    angle_pub_msg.data.data = (float*)allocator.allocate((NUM_SERVOS + 1) * sizeof(float), allocator.state);

    rclc_publisher_init_best_effort(&angle_publisher, &node,
        ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32MultiArray),
        "/arm_joint_angles");

    // --- 话题订阅者: /arm_joint_commands ---
    Serial.println("[micro-ROS] 创建订阅者 /arm_joint_commands ...");
    ...
    rclc_subscription_init_best_effort(&angle_subscriber, &node,
        ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32MultiArray),
        "/arm_joint_commands");

    // // --- 服务: /arm_home ---
    // Serial.println("[micro-ROS] 创建服务 /arm_home ...");
    // memset(&home_req, 0, sizeof(home_req));
    // memset(&home_res, 0, sizeof(home_res));
    // rclc_service_init_best_effort(&home_service, &node,
    //     ROSIDL_GET_SRV_TYPE_SUPPORT(std_srvs, srv, Trigger),
    //     "/arm_home");
    // --- 话题发布者: /sensor_temp_humidity ---
    Serial.println("[micro-ROS] 创建发布者 /sensor_temp_humidity ...");
    memset(&temp_humidity_pub_msg, 0, sizeof(temp_humidity_pub_msg));
    temp_humidity_pub_msg.data.size = 3;
    temp_humidity_pub_msg.data.capacity = 3;
    temp_humidity_pub_msg.data.data = (float*)allocator.allocate(3 * sizeof(float), allocator.state);

    rclc_publisher_init_best_effort(&temp_humidity_publisher, &node,
        ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32MultiArray),
        "/sensor_temp_humidity");

    // 初始化执行器
    Serial.println("[micro-ROS] 初始化执行器...");
    ...
    rclc_timer_init_default(&timer1, &support, RCL_MS_TO_NS(2000), angle_pub_callback);
    rclc_executor_add_timer(&executor, &timer1);
    rclc_timer_init_default(&timer2, &support, RCL_MS_TO_NS(2000), temp_humid_callback);
    rclc_executor_add_timer(&executor, &timer2);

    micro_ros_initialized = true;
    Serial.println("[micro-ROS] 初始化完成, 等待 Agent 连接...");
}



// ---------- ROS 2 订阅回调: 上位机通过 /arm_joint_commands 发送目标角度 ----------
// 消息类型: std_msgs/Float32MultiArray, 4 个 float 分别对应 4 个关节
void angle_cmd_callback(const void *msg_in) {
...
// void home_service_callback(const void *req_in, void *res_out) {
//   int home[NUM_SERVOS] = {90, 90, 90, 90};
//   for (int i = 0; i < NUM_SERVOS; i++) {
...
void angle_pub_callback(rcl_timer_t* timer, int64_t last_call_time) {
  ...
  angle_pub_msg.data.data[0] = (float)stamp;           // 时间戳放第一个
  for (int i = 0; i < NUM_SERVOS; i++) {
    angle_pub_msg.data.data[i + 1] = (float)servoAngles[i];  // 角度往后移
  }
  (void)rcl_publish(&angle_publisher, &angle_pub_msg, NULL);
}
/**
 * @brief 温湿度传感器数据发布回调函数
 *
 * 定时触发，从 AHT30 传感器读取温度和湿度数据，并附加时间戳后发布到 ROS 2 话题
 *
 * @param timer ROS 2 定时器指针（未使用）
 * @param last_call_time 上次调用时间戳（未使用）
 *
 * @note 发布格式：[时间戳(ms), 温度(°C), 湿度(%RH)]
 * @note 话题名称：/sensor_temp_humidity
 * @note 消息类型：std_msgs/Float32MultiArray
 */
...
void temp_humid_callback(rcl_timer_t *timer, int64_t last_call_time)
{
  ...
  temp_humidity_pub_msg.data.data[0] = (float)stamp;           // 时间戳
  temp_humidity_pub_msg.data.data[1] = currentTemp;            // 温度
  temp_humidity_pub_msg.data.data[2] = currentHumidity;        // 湿度
  (void)rcl_publish(&temp_humidity_publisher, &temp_humidity_pub_msg, NULL);
}
...
// ---------- 设置舵机角度函数 ----------
// 参数 channel: LEDC 通道号 (0~3)
// 参数 angle: 目标角度 (0~180)
void setServoAngle(int channel, int angle) {
  if(channel==1)
  {
    int duty = angleToDuty(180-angle);
...
<div id="sliders"></div>
<div class="servo-card" style="margin-top:4px;">
  <div class="servo-header">
    <span class="servo-name">🌡 Temperature</span>
    <span class="servo-angle" id="tempVal">--°C</span>
  </div>
  <div class="servo-header" style="margin-top:6px;">
    <span class="servo-name">💧 Humidity</span>
    <span class="servo-angle" id="humiVal">--%</span>
  </div>
</div>
<div class="btn-row">
...
<script>
// === JavaScript 部分 ===
// 4 个舵机的名称数组, 和 C++ 端一一对应
const NAMES = ["Base","Shoulder","Elbow","Gripper"];
// 每个舵机的最小角度
const MIN = [0,0,0,0];
// 每个舵机的最大角度
const MAX = [180,180,180,180];
// 记录当前 4 个舵机的角度, 初始值 90°
...
function sendAngles() {
  ...
  const p = angles.join(',');                   // 把 4 个角度用逗号拼成字符串, 如 "90,45,120,60"
  ...
  fetch('/state')                               // 向 ESP32 发送 GET /state 请求
    ...
    .then(d=>{                                  // d = {"ip":"192.168.x.x","angles":[90,45,...]}
      if(d.angles) {                            // 如果返回数据中包含 angles 数组
        d.angles.forEach((a,i) => {             // 遍历 4 个角度值
      ...
      if(d.temp !== undefined) document.getElementById('tempVal').textContent = d.temp + '°C';
      if(d.humi !== undefined) document.getElementById('humiVal').textContent = d.humi + '%';
...
function goHome() {
  const home = [90,90,90,90];             // 中立位置的角度值
  angles = [...home];                           // 更新本地角度数组 (展开运算符拷贝)
  home.forEach((v,i) => {                       // 遍历 4 个角度
...
void handleState() {
  ...
  // 结果格式: {"ip":"...","angles":[90,45,...],"temp":25.5,"humi":60.3}
  String json = "{\"ip\":\"" + WiFi.localIP().toString() + "\",\"angles\":[";  // JSON 开头, 写入 IP
  for (int i = 0; i < NUM_SERVOS; i++) {       // 遍历 4 个舵机
    json += String(servoAngles[i]);             // 把角度值追加到 JSON 字符串
    if (i < NUM_SERVOS - 1) json += ",";        // 前 3 个后面加逗号, 最后一个不加
  }
  json += "],\"temp\":" + String(currentTemp, 1);
  json += ",\"humi\":" + String(currentHumidity, 1);
  json += "]}";                                 // 关闭 JSON 数组和对象
...
void setup() {
  ...
  Serial.println("\n=== 4DOF Servo Controller (LEDC PWM) ===");  // 打印启动信息

  // --- 第0步: 初始化 AHT30 温湿度传感器 (I2C) ---
  ...
  if (ahtReady) {
    ...
    currentTemp = temp.temperature;
    currentHumidity = humidity.relative_humidity;
    Serial.printf("AHT30 initialized: %.1f°C, %.1f%%RH\n", currentTemp, currentHumidity);
  } else {
    Serial.println("AHT30 not found! Check wiring (SDA=GPIO4, SCL=GPIO5)");
  }
  ...
  // 每个 GPIO 绑定一个独立的通道, 4 个舵机互不干扰
  for (int i = 0; i < NUM_SERVOS; i++) {        // 循环 4 次, 为每个舵机配置通道
    ledc_channel_config_t channel_conf = {       // 创建通道配置结构体
      .gpio_num = SERVO_PINS[i],                // 舵机信号输出引脚 (GPIO 6~9)
      .speed_mode = LEDC_MODE,                  // 低速模式 (与定时器匹配)
      .channel = (ledc_channel_t)SERVO_CHANNELS[i],  // 通道号 0~3 (强制类型转换)
  ...
  if (WiFi.status() == WL_CONNECTED) {          // 检查是否连接成功
    ...
    // micro-ROS 在 loop() 中自动尝试初始化和重连, 不在这里阻塞
  } else {                                      // 连接失败
  ...
  // micro-ROS 初始化成功后, loop() 中会自动处理 (不需要单独任务)
}

// =================================================================
//  七、loop() - 主循环 (上电后无限重复执行)
//  HTTP 服务器优先, micro-ROS 自动初始化/重连, 不阻塞
// =================================================================
void loop() {
  server.handleClient();                        // 处理来自浏览器的 HTTP 请求 (优先级最高)

  // micro-ROS 自动初始化: WiFi 连上后, 如果还没初始化就尝试 (每10秒试一次)
  if (WiFi.status() == WL_CONNECTED && !micro_ros_initialized) {
    static unsigned long lastInitAttempt = 0;
    if (millis() - lastInitAttempt >= 10000) {  // 每 10 秒尝试一次
      lastInitAttempt = millis();
      Serial.println("[micro-ROS] 尝试初始化...");
      micro_ros_init();                          // 失败也没关系, 下次 loop 会再试
    }
  }

  // micro-ROS: 非阻塞等待 Agent 连接 (每次最多等 5ms)
  if (micro_ros_initialized && !micro_ros_connected) {
    rmw_uros_sync_session(5);                   // 极短超时, 不影响 HTTP 响应
    if (rmw_uros_epoch_synchronized()) {
      micro_ros_connected = true;
      Serial.println("[micro-ROS] Agent 连接成功!");
    }
  }

  // micro-ROS: 已连接则处理消息 (每次最多 spin 5ms)
  ...
  if (millis() - lastPrint >= 10000) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      Serial.println("[IP] WiFi not connected!");
    }
  }
}
