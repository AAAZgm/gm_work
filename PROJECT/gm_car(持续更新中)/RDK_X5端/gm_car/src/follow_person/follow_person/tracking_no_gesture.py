...
class FollowPersonNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('linear_velocity', 0.2)    # 最大前进线速度 m/s（参考官方默认值）
        self.declare_parameter('angular_velocity', 0.4)   # 最大旋转角速度 rad/s（参考官方默认值）
        self.declare_parameter('stop_move_ratio', 0.35)   # 太近停止阈值：目标框宽度占画面宽度的比例
    ...
    def detection_callback(self, msg):
        ...
        if self.track_id is not None and self.track_id != best_track_id:
            # 检测到的最佳目标与之前跟踪的不是同一个（可能是目标切换或 ID 跳变）
            self.activate_count = 0                            # 重置激活计数（需要重新积累信心）
            self.stop_robot()                                  # 目标切换时先停车
        ...
        too_close_top = (y1 < self.img_h * 0.1)            # 框太靠上 → 再走拍不全

        if too_close or too_close_top:
            linear_x = 0.0
            if self.frame_count % 30 == 0:
                self.get_logger().warn('⛔ 目标太近，停止前进')
        elif distance_ratio < self.stop_ratio * 0.5:
            linear_x = self.linear_vel                      # 足够远 → 全速
        else:
            # 中间区域：从全速线性衰减到 0（ratio 越大 → 越慢）
            t = (distance_ratio - self.stop_ratio * 0.5) / (self.stop_ratio * 0.5)
