  ...
  fishbot_interfaces__srv__FishBotConfig_Request__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    fishbot_interfaces__srv__FishBotConfig_Request * data =
      (fishbot_interfaces__srv__FishBotConfig_Request *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!fishbot_interfaces__srv__FishBotConfig_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          fishbot_interfaces__srv__FishBotConfig_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
  ...
  fishbot_interfaces__srv__FishBotConfig_Response__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    fishbot_interfaces__srv__FishBotConfig_Response * data =
      (fishbot_interfaces__srv__FishBotConfig_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    ...
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!fishbot_interfaces__srv__FishBotConfig_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          fishbot_interfaces__srv__FishBotConfig_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
